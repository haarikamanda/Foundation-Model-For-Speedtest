...
class NetFoundEmbeddingsWithMeta:
    def __init__(self, config):
        ...
        self.compressEmbeddings = nn.Linear(config.hidden_size*2, config.hidden_size)
    ...
    def addMetaEmbeddings(self,
        ...
        return self.compressEmbeddings(embeddings)
...
class NetFoundLanguageModelling(NetFoundPretrainedModel):
    ...
    def forward(
        ...
        for i in range(batch_size):
            ...
            for chunk_idx in range(int((seq_len)//self.chunk_size)):
                ...
                #chunking the meta data
                chunk_directions = directions[i, start_idx:end_idx].unsqueeze(0)
                chunk_iats = iats[i, start_idx:end_idx].unsqueeze(0)
                chunk_bytes = bytes[i, start_idx:end_idx].unsqueeze(0)
                # pdb.set_trace()
                ...
                outputs = self.base_transformer(
                    ...
                    direction=chunk_directions,
                    iats=chunk_iats,
                    bytes=chunk_bytes,
                    pkt_count=chunk_bytes,
        ...
        # pdb.set_trace()
        # timestamp = time.strftime("%Y%m%d-%H%M%S")  # e.g., "20241212-153045"

        # Create a unique file name with the timestamp
        # file_name = f'/global/homes/h/haarika/pscratch/network-data-representation/model_output2/window_representations/final_representation_{timestamp}.pt'

        # Save the tensor
        # torch.save(final_representation.cpu(), file_name)
        prediction_scores = self.lm_head(final_representation) 
        # pdb.set_trace()
        burstReps = final_representation[:, ::self.max_burst_length, :].clone().reshape(-1,final_representation.shape[-1])
        burstMetasToBeMasked = burstMetasToBeMasked.reshape(-1,1)
        metaPreds = self.linearMetadataPred(burstReps)
        metaPreds = burstMetasToBeMasked.to(torch.float32)*metaPreds
        # pdb.set_trace()
        metaLabels = (torch.cat([iat_orig.T, bytes_orig.T, bytes_orig.T], dim=1) * burstMetasToBeMasked.expand(-1, 3))

        # metaLabels = (torch.cat([iat_orig, bytes_orig, bytes_orig], dim=1) * burstMetasToBeMasked.expand(-1, 3)).T

        # metaLabels = burstMetasToBeMasked.to(torch.float32) * torch.cat([iat_orig, bytes_orig, bytes_orig], dim=1)

        # metaLabels = burstMetasToBeMasked.to(torch.float32)*torch.hstack([iat_orig, bytes_orig, bytes_orig])
        #direction will be a classification task, -100 is used to not compute loss in pytorch. All the unmasked values will be set to 0, so we remove the 0 directions.
        # pdb.set_trace()
        # direction_orig = direction_orig * burstMetasToBeMasked.to(direction_orig.dtype)
        # direction_orig[direction_orig.to(torch.long)==0]=-100
        # #We have +1 -1 as direction, but for classification we need 0 1. Setting -1 as 0 for classification
        # direction_orig[direction_orig.to(torch.long)==-1]=0
        ...
        # Handle -1 values
        direction_orig[direction_orig == -1] = 0
        ...
        if labels is not None:
            ...
            if not self.no_mlm:
                ...
                metaPreds = metaPreds.reshape(-1)  
                metaLabels = metaLabels.reshape(-1)  
                # pdb.set_trace()
                metaLoss = loss_fct2(metaPreds, metaLabels.to(metaPreds.dtype))
                dirLoss = loss_fct(directn_logits, direction_orig.flatten())
                # print("The loss is ",masked_lm_loss,"for masking")
                # print("The loss is ",metaLoss," for meta")
                # print("The loss is ",dirLoss,"for direction")
                totalLoss += metaLoss
